...
class StringField(Field):
    ...
    def __init__(self, **kw):
        if not 'default' in kw:
            logging.debug("default is not in {0}".format(kw))
            kw['default'] = ''
        if not 'ddl' in kw:
            logging.debug("ddl is not in {0}".format(kw))
            kw['ddl'] = 'varchar(255)'
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        logging.debug("cls is {0}".format(cls))
        logging.debug("name is {0}".format(name))
        logging.debug("bases is {0}".format(bases))
        logging.debug("attrs is {0}".format(attrs))
        # logging.debug(attrs.get("id"))
...
if __name__ == '__main__':
    ...
    # 创建实例:
    user = User( name='Michael22', passwd="passwd1", email="m@itu.edu")
