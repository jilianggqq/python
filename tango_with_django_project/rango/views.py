...
def index2(request):
    ...
    return render(request, 'rango/index2.html', context_dict)
...
def category(request, category_name_slug):
    ...
    try:
        ...
        pages = Page.objects.filter(c_id=category.c_id)
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        logging.debug("next is {0}".format(nextp))
        # Use Django's machinery to attempt to see if the username/password
        # combination is valid - a User object is returned if it is.
        user = authenticate(username=username, password=password)

        logging.debug("user is: {0}".format(user))
        ...
        if user:
            ...
            if user.is_active:
                ...
                login(request, user)
                path = nextp if nextp else '/rango/'
    ...
    # This scenario would most likely be a HTTP GET.
        ...
        logging.debug("next is {0}".format(nextp))
        return render(request, 'rango/login.html', {'next': nextp})
